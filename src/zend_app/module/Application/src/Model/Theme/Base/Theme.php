...
abstract class Theme implements ActiveRecordInterface
{
     ...
     *
     * Note: this column has a database default value of: 'black'
     * @var        string
     */
    protected $color;

    /**
     * @var        ObjectCollection|Note[] Collection to store aggregation of Note objects.
     */
    protected $collNotes;
    ...
    public function setHeight($v)
    {
        ...
        if ($this->height !== $v) {
            $this->height = $v;
            $this->modifiedColumns[ThemeTableMap::COL_HEIGHT] = true;
        }
    ...
    public function setWidth($v)
    {
        ...
        if ($this->width !== $v) {
            $this->width = $v;
            $this->modifiedColumns[ThemeTableMap::COL_WIDTH] = true;
        }
    ...
    public function setBackgroundColor($v)
    {
        ...
        if ($this->backgroundcolor !== $v) {
            ...
            $this->modifiedColumns[ThemeTableMap::COL_BACKGROUNDCOLOR] = true;
        }
    ...
    public function setColor($v)
    {
        ...
        if ($this->color !== $v) {
            $this->color = $v;
            $this->modifiedColumns[ThemeTableMap::COL_COLOR] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ThemeTableMap::translateFieldName('Height', TableMap::TYPE_PHPNAME, $indexType)];
            $this->height = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ThemeTableMap::translateFieldName('Width', TableMap::TYPE_PHPNAME, $indexType)];
            $this->width = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : ThemeTableMap::translateFieldName('BackgroundColor', TableMap::TYPE_PHPNAME, $indexType)];
            $this->backgroundcolor = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : ThemeTableMap::translateFieldName('Color', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ThemeTableMap::COL_HEIGHT)) {
            $modifiedColumns[':p' . $index++]  = '`height`';
        }
        if ($this->isColumnModified(ThemeTableMap::COL_WIDTH)) {
            $modifiedColumns[':p' . $index++]  = '`width`';
        }
        if ($this->isColumnModified(ThemeTableMap::COL_BACKGROUNDCOLOR)) {
            $modifiedColumns[':p' . $index++]  = '`backgroundColor`';
        }
        if ($this->isColumnModified(ThemeTableMap::COL_COLOR)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ThemeTableMap::COL_HEIGHT)) {
            $criteria->add(ThemeTableMap::COL_HEIGHT, $this->height);
        }
        if ($this->isColumnModified(ThemeTableMap::COL_WIDTH)) {
            $criteria->add(ThemeTableMap::COL_WIDTH, $this->width);
        }
        if ($this->isColumnModified(ThemeTableMap::COL_BACKGROUNDCOLOR)) {
            $criteria->add(ThemeTableMap::COL_BACKGROUNDCOLOR, $this->backgroundcolor);
        }
        if ($this->isColumnModified(ThemeTableMap::COL_COLOR)) {
            $criteria->add(ThemeTableMap::COL_COLOR, $this->color);
        }
     ...
     * @return ObjectCollection|Note[] List of Note objects
     * @throws PropelException
     */
    ...
    /**
     * Returns the number of related BaseNote objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related BaseNote objects.
     * @throws PropelException
     */
     ...
     */
    protected function doAddNote(Note $note)
    {
        $this->collNotes[]= $note;
        $note->setTheme($this);
    }
     ...
     * @return ObjectCollection|Note[] List of Note objects
     */
    public function getNotesJoinPosition(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = NoteQuery::create(null, $criteria);
