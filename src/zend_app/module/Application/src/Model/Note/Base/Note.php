...
abstract class Note implements ActiveRecordInterface
{
    ...
    public function setContent($v)
    {
        ...
        if ($this->content !== $v) {
            $this->content = $v;
            $this->modifiedColumns[NoteTableMap::COL_CONTENT] = true;
        }
    ...
    public function setThemeId($v)
    {
        ...
        if ($this->theme_id !== $v) {
            $this->theme_id = $v;
            $this->modifiedColumns[NoteTableMap::COL_THEME_ID] = true;
        }
    ...
    public function setPositionId($v)
    {
        ...
        if ($this->position_id !== $v) {
            $this->position_id = $v;
            $this->modifiedColumns[NoteTableMap::COL_POSITION_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : NoteTableMap::translateFieldName('Content', TableMap::TYPE_PHPNAME, $indexType)];
            $this->content = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : NoteTableMap::translateFieldName('ThemeId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->theme_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : NoteTableMap::translateFieldName('PositionId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->position_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : NoteTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aPosition !== null && $this->position_id !== $this->aPosition->getId()) {
            $this->aPosition = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(NoteTableMap::COL_CONTENT)) {
            $modifiedColumns[':p' . $index++]  = '`content`';
        }
        if ($this->isColumnModified(NoteTableMap::COL_THEME_ID)) {
            $modifiedColumns[':p' . $index++]  = '`theme_id`';
        }
        if ($this->isColumnModified(NoteTableMap::COL_POSITION_ID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(NoteTableMap::COL_CONTENT)) {
            $criteria->add(NoteTableMap::COL_CONTENT, $this->content);
        }
        if ($this->isColumnModified(NoteTableMap::COL_THEME_ID)) {
            $criteria->add(NoteTableMap::COL_THEME_ID, $this->theme_id);
        }
        if ($this->isColumnModified(NoteTableMap::COL_POSITION_ID)) {
            $criteria->add(NoteTableMap::COL_POSITION_ID, $this->position_id);
        }
        if ($this->isColumnModified(NoteTableMap::COL_CREATED_AT)) {
    ...
    {
        if ($this->aTheme === null && ($this->theme_id != 0)) {
    ...
    {
        if ($this->aPosition === null && ($this->position_id != 0)) {
            $this->aPosition = PositionQuery::create()->findPk($this->position_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aPosition->addNotes($this);
             */
        }
    ...
    {
        if (null !== $this->aTheme) {
            $this->aTheme->removeNote($this);
        }
        if (null !== $this->aPosition) {
            $this->aPosition->removeNote($this);
        }
        $this->id = null;
        $this->title = null;
        $this->content = null;
