...
abstract class Position implements ActiveRecordInterface
{
    ...
    public function setXpos($v)
    {
        ...
        if ($this->xpos !== $v) {
            $this->xpos = $v;
            $this->modifiedColumns[PositionTableMap::COL_XPOS] = true;
        }
    ...
    public function setYpos($v)
    {
        ...
        if ($this->ypos !== $v) {
            $this->ypos = $v;
            $this->modifiedColumns[PositionTableMap::COL_YPOS] = true;
        }
    ...
    public function setZpos($v)
    {
        ...
        if ($this->zpos !== $v) {
            $this->zpos = $v;
            $this->modifiedColumns[PositionTableMap::COL_ZPOS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PositionTableMap::translateFieldName('Xpos', TableMap::TYPE_PHPNAME, $indexType)];
            $this->xpos = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PositionTableMap::translateFieldName('Ypos', TableMap::TYPE_PHPNAME, $indexType)];
            $this->ypos = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PositionTableMap::translateFieldName('Zpos', TableMap::TYPE_PHPNAME, $indexType)];
            $this->zpos = (null !== $col) ? (int) $col : null;
            $this->resetModified();
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PositionTableMap::COL_XPOS)) {
            $modifiedColumns[':p' . $index++]  = '`xpos`';
        }
        if ($this->isColumnModified(PositionTableMap::COL_YPOS)) {
            $modifiedColumns[':p' . $index++]  = '`ypos`';
        }
        if ($this->isColumnModified(PositionTableMap::COL_ZPOS)) {
            $modifiedColumns[':p' . $index++]  = '`zpos`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`ypos`':
                        ...
                        break;
                    case '`zpos`':
                        $stmt->bindValue($identifier, $this->zpos, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setXpos($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setYpos($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setZpos($arr[$keys[3]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PositionTableMap::COL_XPOS)) {
            $criteria->add(PositionTableMap::COL_XPOS, $this->xpos);
        }
        if ($this->isColumnModified(PositionTableMap::COL_YPOS)) {
            $criteria->add(PositionTableMap::COL_YPOS, $this->ypos);
        }
        if ($this->isColumnModified(PositionTableMap::COL_ZPOS)) {
            $criteria->add(PositionTableMap::COL_ZPOS, $this->zpos);
        }
     ...
     * @return ObjectCollection|Note[] List of Note objects
     */
    public function getNotesJoinTheme(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = NoteQuery::create(null, $criteria);
